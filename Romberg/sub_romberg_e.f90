...
!********10********20*********30********40********50********60********70**
      subroutine sub_romberg_e(f_any,a,b,e,s)
          ...
          do
              ...
              if(size(tbk)<2)tbk(1,1)=r
              t=0
              t(1:n,1:n)=tbk(1:n,1:n) 
              ! 扩维数组并引入老数组
              n=n+1
              x=(b-a)/(2**(n-1))          ! x 此时作为步长
              sum_newdot=0
              do i=1,2**(n-2) 
                  sum_newdot=sum_newdot+f_any(a+(2*i-1)*x)
              end do 
              t(n,1)=0.5*t(n-1,1)+x*sum_newdot
              do m=2,n
                  t(n,m)=(4**(m-1)*t(n,m-1)-t(n-1,m-1))/&
                          &(4**(m-1)-1)
              end do
              x=abs(t(n,n)-t(n-1,n-1))    ! x 此时作误差
              s=t(n,n)
              deallocate(t,tbk)
